...
Standalone version adapted from ccmux (https://github.com/alvinonmars/claude-code-hub).

Two-layer architecture:
  Layer 1 (regex, automated): Generic patterns (in-repo, safe for public) +
      personal blocklist from ~/.config/privacy/blocklist.txt (gitignored).
...
Setup:
    ...
    3. Create ~/.config/privacy/blocklist.txt with personal patterns (one per line):
         # Family names
         Alice
         Bob
         # Paths
         ...
         # Domains
         myemail@gmail.com
    4. (Optional) Create ~/.config/privacy/allowlist.txt for false-positive overrides
"""
...
# ---------------------------------------------------------------------------
# Configuration — standalone paths (no external dependencies)
# ---------------------------------------------------------------------------

_CONFIG_DIR = Path(os.environ.get(
    "PRIVACY_CONFIG_DIR",
    os.path.expanduser("~/.config/privacy"),
))

BLOCKLIST_PATH = Path(os.environ.get(
    "PRIVACY_BLOCKLIST", str(_CONFIG_DIR / "blocklist.txt")
))

ALLOWLIST_PATH = Path(os.environ.get(
    "PRIVACY_ALLOWLIST", str(_CONFIG_DIR / "allowlist.txt")
))
...
- Contacts: phone numbers, email addresses, messaging IDs
...
- Test fixtures with clearly fake/random data
- Pattern definitions in the scanner itself (regex strings are not PII)
- Generic category labels without real identifying detail
- Publicly known institutional names without accompanying personal detail
- Book content, translations, literary text (this is a book publishing repo)
...
- Real human names: any language, any script (Latin, CJK, Arabic, etc.), including
  partial names, nicknames, and diminutives.
- Phone numbers: any format, any country, including partial numbers
- Email addresses: any format, including ones disguised with [at] or spaces
- Messaging identifiers: WhatsApp JIDs, Telegram usernames, Signal numbers
- Social media handles or usernames that identify a real person

EXCEPTIONS for this repository:
- Author name "姜蓝" (Jiang Lan) is the published book author — NOT PII.
- Translator/editor name in commit messages is public attribution — NOT PII.
- Historical/literary figure names (张学良, 杨宇霆, etc.) are NOT PII.

Be especially alert to non-ASCII names and names split across variables.
""",
    ),
    ReviewerPersona(
...
EXCEPTIONS for this repository:
- GitHub Pages URLs for this repo are public — NOT secrets.
- Discord invite links intentionally published are NOT secrets.
""",
    ),
    ReviewerPersona(
...
not look sensitive in isolation but becomes identifying when combined:

- Schedule details tied to a named or identifiable person
- Location details that narrow down a family's identity
- Health information linked to identifiable individuals
- Financial details linked to an identifiable household
- Routine patterns of a named person

EXCEPTIONS for this repository:
- Book content discussing personal philosophy is literary text — NOT PII.
- The author's public biographical details mentioned in the book are NOT PII.
...
def _compute_review_hash() -> str:
    """Return SHA256 hex digest of review prompt version, all prompt content, and staged diff."""
